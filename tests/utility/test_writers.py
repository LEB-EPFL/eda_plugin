...
from test_event_bus import event_bus, java_settings_event, java_settings_event_w_save_loc
...
def test_full(writer_plugin, java_settings_event):

    # Acquisition is started
    writer_plugin.event_bus.acquisition_started_event.emit(None)
    writer_plugin.event_bus.acquisition_started_event.emit(java_settings_event)
    assert writer_plugin.settings is not None

    # Image arrives
    writer_plugin.event_bus.new_image_event.emit(PyImage(np.random.rand(1024, 1024), 0, 0, 0, 1023))
